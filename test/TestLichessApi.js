...
const accountResponse = {
  status: 200,
  response: { id: "bot-o-tron", username: "bot-o-tron" },
}
...
tap.test("acceptChallenge", async function (t) {
  assertRequest(
    t,
    "post",
    new RegExp(`api/challenge/${challengeId}/accept`),
    okResponse
  )
...
tap.test("declineChallenge", async function (t) {
  assertRequest(
    t,
    "post",
    new RegExp(`api/challenge/${challengeId}/decline`),
    okResponse
  )
...
tap.test("makeMove", async function (t) {
  ...
  assertRequest(
    t,
    "post",
    new RegExp(`api/bot/game/${gameId}/move/${move}`),
    okResponse
  )
...
tap.test("abortGame", async function (t) {
  assertRequest(
    t,
    "post",
    new RegExp(`api/bot/game/${gameId}/abort`),
    okResponse
  )
...
tap.test("resignGame", async function (t) {
  assertRequest(
    t,
    "post",
    new RegExp(`api/bot/game/${gameId}/resign`),
    okResponse
  )
...
tap.test("chat", async function (t) {
  assertRequest(
    t,
    "post",
    new RegExp(`api/bot/game/${gameId}/chat`),
    okResponse
  )
